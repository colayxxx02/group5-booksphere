...
switch ($type) {
    case 'borrows':
        ...
        if (empty($daily_borrows)) {
            ...
            $total_days = count($daily_borrows);
            $html = '<div style="margin-bottom:10px; color:#888; font-size:13px;">Showing ' . $total_days . ' day' . ($total_days > 1 ? 's' : '') . ' with activity</div>';
    ...
    case 'returns':
        ...
        if (empty($daily_returns)) {
            ...
            $total_days = count($daily_returns);
            $html = '<div style="margin-bottom:10px; color:#888; font-size:13px;">Showing ' . $total_days . ' day' . ($total_days > 1 ? 's' : '') . ' with returns</div>';
            $html .= '<div class="details-list">';
    ...
    case 'category':
        ...
        if (empty($categories)) {
            ...
            $html = '<div style="margin-bottom:10px; color:#888; font-size:13px;">Showing ' . $total_categories . ' categor' . ($total_categories > 1 ? 'ies' : 'y') . '</div>';
            $html .= '<div class="details-list">';
    ...
    case 'members':
        ...
        if (empty($new_members)) {
            ...
            $total_members = count($new_members);
            $html = '<div style="margin-bottom:10px; color:#888; font-size:13px;">Total: ' . $total_members . ' new member' . ($total_members > 1 ? 's' : '') . '</div>';
            $html .= '<div class="details-list">';
